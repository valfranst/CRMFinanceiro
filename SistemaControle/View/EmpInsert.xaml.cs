...
{
    public partial class EmpInsert : UserControl
    {
        ...
        public EmpInsert(MainWindow mainWindow, Cliente cliente2)
        {
            InitializeComponent();
            this.mw = mainWindow;
        ...
        private void btCadCliente_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (cliente.IdCliente > 0)
                {
                    ClienteView cliView = new ClienteView(mw, cliente.IdCliente);
        ...
        private void btFichaCliente_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (cliente.IdCliente > 0)
                {
                    EmpView empView = new EmpView(mw, cliente.IdCliente);
                ...
                using (RepresaContext dc = new RepresaContext())
                {
                    ...
                    if (idEmprestimo > 0)
                    {
                        ...
                        if (parcelas.Count > 0)
                        {
                            ...
                            Espelho espelho = new Espelho { Acao = "Inserir", Tipo = emprestimo.GetType(), Dado = JsonConvert.SerializeObject(emprestimo) };
                            Resultado resultado = fire.setFileJson(espelho);

                            foreach (var parcela1 in parcelas)
                            {
                                parcela1.IdParcela = 0;
                                espelho = new Espelho { Acao = "Inserir", Tipo = parcela1.GetType(), Dado = JsonConvert.SerializeObject(parcela1) };
                                espelhos.Add(espelho);
                            }
                            resultado = fire.setFileJson(espelhos);
                        ...
                        else
                        {
                            ...
                            dc.SaveChanges();
                            Espelho espelho = new Espelho { Acao = "Excluir", Tipo = empDelete.GetType(), Dado = JsonConvert.SerializeObject(empDelete) };
                            Resultado resultado = fire.setFileJson(espelho);
